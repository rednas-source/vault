...
const KINDS = ['model','texture','material','animation','audio','vfx','scene','ui','other'];
const ROLES = ['model','rig','animation','texture','preview','reference','source','documentation','other'];
...
function metadata(input) {
  ...
  return {schemaVersion:1,name,kind:KINDS.includes(input.kind)?input.kind:'model',category:text(input.category,80)||'Unsorted',
...
class AssetLibrary {
  ...
  async list(query={}){
    await this.scan();let rows=[...this.items.values()];
    ...
    return {items:rows.slice(offset,offset+limit).map(({files,notes,provenance,importKey,...r})=>r),total,libraryTotal:this.items.size,offset,limit,facets,issues:this.errors};
  }
  ...
  create(input){return this.exclusive(async()=>{
    ...
    if(key){const existing=[...this.items.values()].find(r=>r.importKey===key);if(existing){const current=await this.readPackage(existing.rel);this.items.set(current.id,current);return {...current,existing:true};}}
...
function mountAssetRoutes(app,{root,auth,canUse,note}){
  ...
  const route=fn=>async(req,res)=>{try{await fn(req,res);}catch(e){res.status(e.status||400).json({error:e.status?e.message:'Could not read the asset library. Check that storage is available.'});}};
  app.get('/api/assets',auth,access,route(async(req,res)=>res.json(await library.list(req.query))));
