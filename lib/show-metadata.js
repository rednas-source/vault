...
function createShowMetadata({ cacheDir, lookupTmdb, fetchImpl = fetch, now = Date.now, requestGap = 600 }) {
  ...
  async function resolve(identity, version, force) {
    ...
    data = { ...data, ...(failed ? { reason: 'The metadata provider could not be reached. Try scanning again later.' } : {}) };
    if (!failed) delete data.reason;
  ...
  function get(file, { force = false } = {}) {
    ...
    if (!pending.has(identity.key)) {
      const job = resolve(identity, generation, force).finally(() => { if (pending.get(identity.key) === job) pending.delete(identity.key); });
