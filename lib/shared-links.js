...
async function listSharedLinks(root, shares, user, accessForOwner, now = Date.now()) {
  ...
  for (const [id, share] of visible) {
    const folder = share.kind === 'folder' && share.writable === true;
    let status = share.expires && now >= share.expires ? 'expired'
      : !folder && share.maxUses && (share.uses || 0) >= share.maxUses ? 'exhausted' : 'active';
    const allowed = accessForOwner(share.by);
    if (status === 'active') {
      if (!allowed.includes(share.rel.split('/')[0])) status = 'unavailable';
      else {
        try {
          const entry = await resolveEntry(root, share.rel, allowed);
          if (folder ? !entry.stat.isDirectory() : !entry.stat.isFile()) status = 'missing';
        } catch { status = 'missing'; }
      }
    }
    result.push({ id, rel: share.rel, kind: folder ? 'folder' : 'file', writable: folder,
      by: share.by, label: share.label || '', created: share.created,
      expires: share.expires || 0, maxUses: folder ? 0 : (share.maxUses || 0), uses: share.uses || 0,
      remaining: folder ? null : (share.maxUses ? Math.max(0, share.maxUses - (share.uses || 0)) : null),
