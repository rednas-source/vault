...
const storage = multer.diskStorage({
  destination: async (req, file, cb) => {
    const shelf = SHELVES.includes(req.body.shelf) ? req.body.shelf : shelfFor(file.originalname);
...
const upload = multer({ storage, limits: { fileSize: (config.maxFileGB || 64) * 1024 ** 3 } });
...
app.get('/api/session', (req, res) => {
  ...
  res.json(u ? { user: u.name, role: u.role } : { user: null });
});
...
app.get('/api/files', auth, async (req, res) => {
  ...
  res.json({ files: out, disk, shelves: SHELVES });
});
...
app.get('/api/stream/*', auth, (req, res) => sendFile(req, res, { download: false }));
app.get('/api/download/*', auth, (req, res) => sendFile(req, res, { download: true }));
...
(async () => {
  ...
  app.listen(PORT, config.bind || '0.0.0.0', () => {
    console.log(`DARKSTAR listening on ${config.bind || '0.0.0.0'}:${PORT}`);
