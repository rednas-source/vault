...
- Search and combine type, category, style, collection, format, review and rig/animation filters. Saved assets persist privately on the Vault account across browsers and devices. Existing browser-local saves migrate when that browser next opens Assets. Results are paginated; models load only on explicit 3D inspection.
...
- [ASSETS.md](ASSETS.md) is the operational reference for package schema, upload recovery, access and limits. Version metadata is descriptive, and declared checksums are not universally verified during upload.
- Assets has its own submitted search field, including tag matching, and browser-local per-account small/medium/large grid or list preferences. Selection updates existing cards and previews in place without fetching the catalog. Select page affects the visible page; batches support up to 200 packages.
- Type defaults to matching package types and connected content, including embedded GLB textures and animations. Belongs to model limits matches to model packages; Standalone assets matches the package's own type. Related files stay attached to their model card. VFX and Source files are explicit types, with VFX also a file role; engine-specific VFX is stored, not executed in the browser.
- Bulk editing changes category, style, collection, review state and tags while preserving geometry and file contents. Blank fields remain unchanged. Revision checks reject stale edits; storage failures report partial completion honestly. Trash hides packages through reversible metadata and supports restoration; it neither erases files nor revokes existing file links.
- The 3D workbench uses an integrated bottom icon dock, content-width expansion and browser fullscreen. Exit 3D stays at the top-right during loading, inspection and fullscreen and returns directly to the image. Mesh mode is the default; Animate reveals motion controls only for files with clips. Textured, solid, wireframe and skeleton overlays aid inspection. Clip seeking, 1/30-second stepping, speed, loop and In/Out ranges are temporary preview controls, not saved or destructive animation editing.
- Prepared imports automatically add missing files to exactly one active package matching sourceAssetId and collection. Ordinary folders match their folder name and collection. Preserve its identity, name, tags, primary GLB and preview; skip unchanged paths and stop on conflicting file sizes. Successful updates require review. The optional Amber Road source add-on contains 35 editable Blender conversions from approved GLBs, with packed images and 12 Blender-confirmed rigs/animation sets. These are conversions, not original pre-texturing sculpt projects.
- Card right-click and the persistent three-dot Actions button expose Open, Edit details, Save to favorites, Get asset and Delete, with keyboard menu navigation. Detail also exposes Delete. Confirmation names the asset, file count and size before permanently deleting the whole package and connected files; stale revisions are rejected. Existing bulk Trash remains reversible.
- Saving a favorite changes the account preference without editing package metadata or remounting the detail image/3D preview. The Saved-only gallery refreshes its filtered results after a change. Asset favorites remain private per account; this does not change Listen's browser-local favorites.
- Image inspection offers preview/reference/texture image selection, a height slider and desktop resize handle, expansion, browser fullscreen and return from 3D within the same asset. These controls do not modify the source images.
- The question-circle beside Add assets in the gallery and import form opens an inline preparation guide. A folder tree and grouped-card example explain one manifest per asset, related files, tags, outer-folder selection and source-ID matching; Download example asset.json supplies a recipe whose paths must be adapted to real files.
