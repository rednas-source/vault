...
- The 3D workbench keeps controls inside its panel, with content-width expansion and browser fullscreen. Textured, solid, wireframe and skeleton overlays aid inspection. Clip seeking, 1/30-second stepping, speed, loop and In/Out ranges are temporary preview controls, not saved or destructive animation editing.
- Prepared imports can add files to exactly one active package matching sourceAssetId and collection. Preserve its identity, name, tags, primary GLB and preview; skip unchanged paths and stop on conflicting file sizes. Successful updates require review. The optional Amber Road source add-on contains 35 editable Blender conversions from approved GLBs, with packed images and 12 Blender-confirmed rigs/animation sets. These are conversions, not original pre-texturing sculpt projects.
